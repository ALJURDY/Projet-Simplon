...
select h.nom as "Liste des habitants (noms)"
from habitant h
         join absorber a on a.num_hab = h.num_hab
         join potion p on a.num_potion = (select p.num_potion
                                          from potion p
...
from habitant h
    ...
    (select array_agg(p2.num_potion)
     from potion p2
...
select distinct t.date_prise as "Date de première prise de trophée"
from trophee t
order by t.date_prise asc
limit 1;

--21. Nombre de louches de Potion magique n°2 (c'est le libellé de la potion) absorbées. (19)
select sum(a.quantite) as "Nombre de louches de Potion magique n°2"
from potion p
         join absorber a on a.num_potion = p.num_potion
where p.lib_potion = 'Potion magique n°2';

--22. Superficie la plus grande. (895)
select r.superficie as "Superficie la plus grande"
from resserre r
order by r.superficie desc
limit 1;
...
SELECT v.nom_village,
       (SELECT COUNT(h.nom) FROM habitant h WHERE v.num_village = h.num_village)
           AS nombre
FROM village v;
...
select p.nom_province, round(avg(h.age), 0) as "Moyenne d'âge des habitans"
from province p
...
select h.nom as "Nom d'habitant", count(a.quantite) as "Nombre de potions différentes absorbées"
from potion p
         join absorber a on a.num_potion = p.num_potion
         join habitant h on h.num_hab = a.num_hab
group by h.nom;
...
select h.nom as "Nom des habitants"
from potion p
         join absorber a on a.num_potion = p.num_potion
         join habitant h on h.num_hab = a.num_hab
...
select v.nom_village as "Noms des villages dans lesquels on trouve une resserre"
from village v
...
select h.nom, count(t.num_preneur) as trophées
from habitant h
         join trophee t on h.num_hab = t.num_preneur
group by h.nom
HAVING COUNT(t.num_preneur) > (select count(t.num_preneur)
                               from trophee t
                                        join habitant h on t.num_preneur = h.num_hab
                               where h.nom = 'Obélix');
